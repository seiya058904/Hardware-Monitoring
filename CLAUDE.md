...
No test suite exists. Verify changes by running `python app.py` on a Windows machine.
...
# Also copy to project root for direct running:
cp "dist/Hardware Monitoring/Hardware Monitoring.exe" ./
cp -r "dist/Hardware Monitoring/_internal" ./
...
- **Config**: JSON file (`config.json`) alongside the EXE. `load_config()` at ~line 2122 handles defaults, validation (opacity clamp, min refresh 300ms, font scale [0.9,1.4]), and legacy mojibake migration for Chinese theme/mode strings.
