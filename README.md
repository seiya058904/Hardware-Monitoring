...
运行时配置和日志保存在 `%LOCALAPPDATA%\\Hardware Monitoring`。构建依赖版本、第三方来源和 SHA-256 记录见 `requirements-*.txt`、`THIRD_PARTY_NOTICES.md`；可运行 `powershell -ExecutionPolicy Bypass -File scripts\\fetch-dependencies.ps1` 获取并校验固定版本的二进制依赖。
